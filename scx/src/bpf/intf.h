...
enum mlfq_consts {
	...
	/*
	 * Wakeup-classification cadence, in nsecs. The gauge decay over the
	 * accumulated sleep runs at most once per window per task. Between two
	 * full classifications only the boost and hysteresis decide, the cheap
	 * comparisons that carry the wakeup latency. The window is a small
	 * fraction of the one-second aging period, so a wakeup burst is
	 * classified on one decay and the aging stay is unaffected. The decay
	 * itself sees the whole sleep since the last classification, so the band
	 * edge is still reached on the same total sleep; only the cadence
	 * drops to one division per window.
	 */
	MLFQ_CLASSIFY_INTERVAL_NS	= (4ULL * NSEC_PER_MSEC),
};
...
struct task_ctx {
	...
	u64 next_classify_at;		/* next full-wakeup classification time */
 ...
 * rtdl_state_stor array map. flags reflects whether a realtime task
 * currently holds the CPU (see ops.cpu_release()/cpu_acquire() in
 * rtdl.bpf.c), which decides whether the CPU is treated as unavailable
 * to the SCX classes. last_drain_at rate-limits the evacuation pass of
 * the takeover path.
 */
...
/*
 * Op-latency histogram. Each slot (MLFQ_OP_LAT_* below) charges a
 * sample (every MLFQ_OP_LAT_SAMPLE_N-th op, see below) of the wall
 * time its callback spends running into one of eight buckets that delimit the
 * elapsed microseconds. The buckets are [0, 2), [2, 5), [5, 10), [10, 20),
...
/*
 * Op-latency sampling cadence. The histogram charges only every
 * MLFQ_OP_LAT_SAMPLE_N-th op of each slot. The wall-time read and
 * the bucket store are therefore paid once per window instead of per callback;
 * the per-CPU per-op sample counter gates on a power of two so the test
 * is a mask. A low rate still resolves the distribution's tail, which is
 * the shift the histogram exists to catch.
 */
enum mlfq_op_lat_sample_consts {
	MLFQ_OP_LAT_SAMPLE_SHIFT	= 3,
	MLFQ_OP_LAT_SAMPLE_N		= (1U << MLFQ_OP_LAT_SAMPLE_SHIFT),
};

/* One op's latency histogram, a BPF_MAP_TYPE_PERCPU_ARRAY value.
 * sampled_cnt is the CPU-local counter that gates which ops the histogram
 * samples (see MLFQ_OP_LAT_SAMPLE_N). */
struct mlfq_op_lat {
	u64 sampled_cnt;
...
/**
 * mlfq_place_consts - The weight-scaled placement constants, computed once.
 * @q: The queue being placed into.
 * @weight: Task weight.
 * @limit: Out, the lag bound (calc_delta_fair(max_slice + TICK, weight)).
 * @vslice: Out, the virtual slice (calc_delta_fair(max_slice, weight)).
 *
 * Both constants scale the queue's request size by the task weight, so
 * each is a division. Computing them once per placement and sharing them
 * between the deadline and the commit avoids repeating the divisions.
 */
static __always_inline void mlfq_place_consts(const struct queue_ctx *q,
					      u32 weight, u64 *limit,
					      u64 *vslice)
{
	*limit = mlfq_lag_limit(q, weight);
	*vslice = calc_delta_fair_bpf(q->max_slice_ns, weight);
}
 ...
 * @limit: The lag bound from mlfq_place_consts().
 * @vslice: The virtual slice from mlfq_place_consts().
 *
 ...
 *   deadline     = vruntime_new + vslice
 *
 * The two weight-scaled constants are computed once per placement
 * (mlfq_place_consts) and shared by the deadline and the commit, so the
 * divisions are not repeated.
 *
					     ...
					     struct task_ctx *tctx)
{
	 ...
	 */
	mlfq_place_consts(q, (u32)tctx->weight, &limit, &vslice);
	deadline = mlfq_place_entity_deadline(q, tctx, limit, vslice);
...
/**
 * mlfq_op_lat_begin - Open a sampled op-latency window.
 * @op: The MLFQ_OP_LAT_* slot.
 *
 * Advances the CPU-local sample counter of the op slot and returns a
 * start timestamp when this op is one of the every-MLFQ_OP_LAT_SAMPLE_N
 * that the histogram charges, or 0 when the op is skipped so the caller
 * does not pay the wall-time read. The per-CPU counter never contends.
 */
static __always_inline u64 mlfq_op_lat_begin(u32 op)
{
	...
	if (!lat)
		return 0;
	if ((lat->sampled_cnt++ & (MLFQ_OP_LAT_SAMPLE_N - 1)) != 0)
		return 0;
	return scx_bpf_now();
}

/**
 * mlfq_op_lat_charge - Charge one sampled op into its latency histogram.
 * @op: The MLFQ_OP_LAT_* slot.
 * @start_ns: 0 when the op was not sampled, or scx_bpf_now() from
 * mlfq_op_lat_begin() for a sampled op.
 *
 * A zero start_ns (the op fell between two sampling ticks) is a no-op,
 * so the hot wakeup path keeps capturing the timestamp only on a sampled op
 * and never pays the histogram bucket store otherwise. For a sampled op it
 * computes the elapsed time and increments the covering bucket of the per-CPU
 * histogram. A failed map lookup is a no-op. The per-CPU counters never
 * contend, and the Rust front-end sums the CPUs for the stats output.
 */
