...
impl<'a> Scheduler<'a> {
    ...
    fn get_metrics(&self) -> Metrics {
        ...
        Metrics {
            ...
            demotions: s[6],
            aging_boosts: s[7],
            short_sleep_boosts: s[8],
            preemption_kicks: s[9],
            cpuperf_boosts: s[10],
            steals: s[11],
            steals_same_llc: s[12],
            steals_cross_llc: s[13],
            keep_running: s[14],
            enq_no_tctx: s[15],
            enq_bad_weight: s[16],
            enq_no_deadline: s[17],
            enq_fastpath: s[18],
            enq_regular: s[19],
            enq_pinned_idle: s[20],
            enq_pinned_busy: s[21],
            enq_pinned_global: s[22],
            rt_takeovers: s[23],
            rt_evacuations: s[24],
            rt_redirects: s[25],
            rt_reenqs: s[26],
    ...
    /// Sum the per-CPU mlfq_stats slots into a flat u64 array in struct
    ...
    fn read_op_lat(&self) -> Vec<u64> {
        ...
        for op in 0..nr_ops {
            let cpu_values = self
                ...
                .unwrap_or_default();
            for chunk in cpu_values {
                // The per-CPU value leads with the sample gate (sampled_cnt)
                // before the bucket vector; skip it so the histogram reads land
                // on the bucket array.
                for (b, slot) in chunk.chunks_exact(8).skip(1).take(buckets).enumerate() {
